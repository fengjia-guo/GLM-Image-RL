...
class TrainingConfig:
    ...
    def __post_init__(self):
        ...
        if self.lora_alpha < self.lora_rank:
            print(f"Warning: lora_alpha ({self.lora_alpha}) < lora_rank ({self.lora_rank}). "
...
class GlmImageLoraTrainer:
    ...
    def compute_loss(self, batch: Dict[str, Any]) -> torch.Tensor:
        ...
        image_grid_thw = image_inputs["image_grid_thw"].to(device)  # [B, 3] for T2I (one grid per image)
        ...
        with torch.no_grad():
            ...
            large_image_tokens = base_model.get_image_tokens(image_features, image_grid_thw)
        ...
        if is_t2i:
            ...
            for i in range(batch_size):
                t, h, w = image_grid_thw[i].tolist()
                num_tokens = t * h * w
                large_tokens_i = large_image_tokens[token_offset:token_offset + num_tokens]
            ...
            target_image_tokens = torch.cat([large_image_tokens, small_image_tokens], dim=0)
        else:
        ...
        config = peft_model.config if hasattr(peft_model, "config") else base_model.config
        ...
        for i in range(batch_size):
            ...
            for j in range(grids_per_sample):
                ...
                h, w = image_grid_thw[grid_idx, 1].item(), image_grid_thw[grid_idx, 2].item()
                total_tokens += h * w
        ...
        for i in range(batch_size):
            ...
            img_tokens = target_image_tokens[token_offset:token_offset + num_img_tokens]
            ...
            labels = torch.cat([
                torch.full((len(text_ids) + 1,), -100, device=device, dtype=torch.long),  # text + <image_start>
                img_tokens,
                torch.tensor([image_end_token_id], device=device),
            ])
        ...
        padded_attention_mask = torch.zeros((batch_size, max_len), device=device, dtype=torch.long)
...
def parse_args():
    ...
    # Model
    parser.add_argument("--model_path", type=str, default="zai-org/GLM-Image",
