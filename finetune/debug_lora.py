...
def load_and_check_weights(lora_path: str):
    ...
    for name, tensor in weights.items():
        ...
        for part in parts:
            if part in ["lora_A", "lora_B"]:
                module_type = parts[parts.index(part) - 1] if parts.index(part) > 0 else "unknown"
                break
    ...
    for module, stats in weight_stats.items():
        ...
        logging.info(f"    {module}: {stats['params']:,} params, avg_norm={avg_norm:.4f}, max_norm={max_norm:.4f}")
        
        if max_norm > 100:
            logging.warning(f"    ⚠ High weight norm in {module} - may indicate training issues")
...
def test_model_loading(model_path: str, lora_path: str):
    ...
    try:
        ...
        if layer_counts:
            ...
            logging.info(f"    Layer range: {min(layer_counts.keys())} - {max(layer_counts.keys())}")
...
def test_simple_generation(model_path: str, lora_path: str = None, device: str = "cuda"):
    ...
    try:
        ...
        logging.info(f"  Image start token ({image_start_id}) present: {has_image_start}")
...
def main():
    ...
    if not args.skip_generation:
        ...
        results["base_gen"] = test_simple_generation(args.model_path, lora_path=None, device=args.device)
        
        logging.info("\n  Testing LoRA model generation...")
        results["lora_gen"] = test_simple_generation(args.model_path, lora_path=args.lora_path, device=args.device)
